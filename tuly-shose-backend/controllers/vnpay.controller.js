...
exports.ipn = async (req, res) => {
  ...
  if (!pending) {
    return res.status(404).json({ RspCode: "01", Message: "Pending order not found" });
  }

  const now = new Date();
  const totalAmount = pending.orderItems.reduce(
    (sum, item) => sum + item.quantity * item.price_after_discount,
    0
  ) + pending.shippingFee;
  ...
  const newOrder = await Order.create({
    ...
    shipping_info: {
      full_name: pending.userInfo.fullName,
      phone: pending.userInfo.phone,
      email: pending.userInfo.email || null,
      address: pending.userInfo.address,
    },
    order_date: now,
    delivery_date: pending.deliveryDate,
    order_status_id: new mongoose.Types.ObjectId(ORDER_PAID_STATUS_ID),
    total_amount: totalAmount,
    payment_status: "Đã thanh toán",
    order_note: pending.orderNote,
    create_at: now,
    update_at: now,
  });

  const orderDetails = pending.orderItems.map((item) => ({
  ...
  if (pending.user_id && pending.isFromCart) {
    await CartItem.deleteMany({ user_id: new mongoose.Types.ObjectId(pending.user_id) });
  }

  if (pending.userInfo.email) {
    const mailOptions = {
      from: process.env.EMAIL_USER,
      to: pending.userInfo.email,
...
`, // Giữ nguyên phần HTML như bạn viết, thay đổi các biến sang pending.*
    };
