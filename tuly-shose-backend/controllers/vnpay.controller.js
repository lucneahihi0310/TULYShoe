...
// Test VNPAY URL connectivity
async function testVnpayUrl() {
  return new Promise((resolve) => {
    const req = https.get(vnp_Url, (res) => {
      console.log(`VNPAY URL Status: ${res.statusCode}`);
      resolve(res.statusCode);
    });
    req.on("error", (err) => {
      console.error("VNPAY URL Error:", err.message);
      resolve(null);
    });
  });
}
...
exports.createPayment = async (req, res) => {
  try {
    // Test VNPAY URL before proceeding
    const vnpayStatus = await testVnpayUrl();
    if (!vnpayStatus || vnpayStatus >= 400) {
      console.error("VNPAY URL unreachable, status:", vnpayStatus);
      return res.status(500).json({ message: "Không thể kết nối tới VNPAY." });
    }
    ...
    // Prepare order data to encode in JWT
    const orderData = {
      orderItems,
      userInfo,
      paymentMethod,
    ...
    // Encode order data into JWT
    const jwtToken = jwt.sign(orderData, process.env.JWT_SECRET, { expiresIn: "1h" });
...
exports.ipn = async (req, res) => {
  try {
    ...
    // Decode JWT to retrieve order data
    let orderData;
    try {
      orderData = jwt.verify(orderInfo, process.env.JWT_SECRET);
    } catch (err) {
      console.warn("Invalid JWT token:", err);
      return res.status(200).json({ RspCode: "98", Message: "Invalid JWT token" });
    }

    const { orderItems, userInfo, paymentMethod, orderNote, shippingFee, user_id, isFromCart, totalAmount } = orderData;
    ...
    if (vnpResponseCode === "00") {
      ...
      const newOrder = await Order.create({
        ...
        order_status_id: new mongoose.Types.ObjectId("60a4c8b2f9a2d3c4e5f6a882"),
      ...
      // Delete cart items if applicable
