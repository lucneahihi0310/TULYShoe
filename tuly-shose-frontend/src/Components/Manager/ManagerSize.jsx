...
const ManagerSize = () => {
    ...
    const searchCategory = categories.filter((c) => {
        const findCategoryByName = c.size_name.toLowerCase().includes(filterCategoryName.toLowerCase());
                                ...
                                { required: true, message: "Please enter size name" },
                                {
                                    validator: (_, value) => {
                                        const isDuplicate = categories.some(
                                            (cat) =>
                                                cat.size_name.trim().toLowerCase() === value?.trim().toLowerCase() &&
                                                cat._id !== edittingRow
                                        );
                                        return isDuplicate
                                            ? Promise.reject("This size name already exists!")
                                            : Promise.resolve();
                                    }
                                }
                            ]}>
    ...
    return (
        <div style={{ borderRadius: '20px', padding: '10px', backgroundColor: '#f7f9fa', width: "100%" }}>
            <Row gutter={16} style={{ padding: '10px' }}>
                ...
                <Col span={8} offset={4}>
                    <Input placeholder="Search size..." prefix={<SearchOutlined />} onChange={(e) => setFilterCategoryName(e.target.value)} />
                </Col>
                <Col span={2} offset={1}>
                    <Select
